...
#   2. sets up rootful podman (userns=host — the only container runtime that
#      works inside AI2 Beaker jobs; apptainer cannot run there at all)
#   3. ensures the 10 shared base images exist (weka tar cache, else podman
#      build from rl_data/containers/docker/) while vLLM loads
#   4. serves a model with vLLM on the local GPUs (GLM-5.2 recipe on B300,
#      generic profile on H100)
#   5. runs rl_data.generate_solutions --container-runtime podman against it
#      (the SFT warm-start rollouts), syncing summaries to the gantry results
#      dataset as it goes
#
# The inner script is scripts/beaker/run_gen_solutions_in_job.sh.
#
# Usage:
...
BEAKER_IMAGE="${BEAKER_IMAGE:-}"   # default derived from HARDWARE below
...
Options:
  ...
  --gpus N               GPUs (default: 8; 0 for --build-images-only)
  ...
  --use-deep-gemm        enable DeepGEMM JIT FP8 kernels (default off: needs a
                         cu129+ toolchain; CUTLASS fallback is precompiled)
  ...
  --build-images-only    CPU-only job: ensure the 10 base images exist in the
                         weka tar cache (podman build if missing) and exit.
                         Podman builds work on holmes itself — validated by
  ...
  --budget BUDGET        beaker budget (default: ${BUDGET})
  --workspace WS         beaker workspace (default: \$BEAKER_WORKSPACE or ai2/oe-agents-holmes)
  ...
  --weka MOUNT           weka mount as src:dst (default: $WEKA_MOUNT);
                         'none' to disable (HF + image caches then job-local!)
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --build-images-only) BUILD_IMAGES_ONLY=1; shift ;;
...
if [ "$BUILD_IMAGES_ONLY" = "1" ] && [ "$GPU_COUNT_SET" = "0" ]; then
    GPU_COUNT=0
fi

if [[ "$BUILD_IMAGES_ONLY" != "1" && "$HARDWARE" == "h100" && "$VLLM_MODEL" == *GLM-5.2* ]]; then
    cat >&2 <<'EOF'
...
if [ "$BUILD_IMAGES_ONLY" = "1" ]; then
    JOB_NAME="${JOB_NAME:-build-base-images}"
else
...
cat <<EOF
=== Launching SFT solution generation on Beaker (podman runtime) ===
  ...
  Parsers:      tool=${VLLM_TOOL_CALL_PARSER:-<auto>} reasoning=${VLLM_REASONING_PARSER:-<auto>}  MTP=${VLLM_ENABLE_MTP} DeepGEMM=${VLLM_USE_DEEP_GEMM}
  ...
  Podman:       image_cache=${IMAGE_CACHE_DIR:-<in-job default>} build_images_only=${BUILD_IMAGES_ONLY}
  Job:          ${BEAKER_NAME}  workspace=${BEAKER_WORKSPACE}  priority=${PRIORITY}  min_runtime=${MIN_RUNTIME:-<none>}
...
GANTRY_CMD=(
    ...
    --name "$BEAKER_NAME"
    --description "SFT solution generation (${TASKS_HF_DATASET}) with ${SERVED_MODEL_NAME} via vLLM + podman on ${HARDWARE}"
    --ref "$REPO_GIT_REF"
    --cluster "$CLUSTER"
    --gpus "$GPU_COUNT"
    --priority "$PRIORITY"
