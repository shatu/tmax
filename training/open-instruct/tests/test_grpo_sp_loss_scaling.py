...
def log_softmax_and_gather(logits, labels):
...
def test_sharded_gradients_match_unsharded(zero_stage, sp_size, mode, uneven):
    ...
    old_logprobs = log_softmax_and_gather(head(backbone(inputs)), labels).detach()
    weights = masks.float()
    counts = {}
    if mode == "sequence":
    ...
    logprobs = log_softmax_and_gather(head(backbone(inputs)), labels)
            ...
            for sp in range(sp_size):
                ...
                for step in range(2):
                    ...
                    if tiled:
                        ...
                        with patch.dict(
                            ns, _sequence_id_counts=Mock(return_value=(counts.get((step, dp), torch.zeros(0)), mask))
                        ):
                        ...
                        local_logprobs = log_softmax_and_gather(local_head(hidden), local_labels)
                        loss = (
