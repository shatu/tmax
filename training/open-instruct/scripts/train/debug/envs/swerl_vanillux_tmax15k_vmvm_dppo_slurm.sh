...
# Portability: this mkdir is unconditional and the script runs under `set -e`, so a
# collaborator without write access to /checkpoint/comem dies here ~15 s in with no
# other output. Made overridable; default is unchanged for Rulin's runs.
# Proposed upstream on collab PR #1 (Oscar, 2026-08-30).
mkdir -p "${TMAX_LOGDIR:-/checkpoint/comem/rulin/logs/swerl_tmax15k_vmvm_dppo}"
...
# Local SIF pool: without this, apptainer/sandfleet backends hand workers raw
# docker refs and every lease tries a registry pull (blocked egress → exit=255).
export SWERL_APPTAINER_SIF_DIR="${SWERL_APPTAINER_SIF_DIR:-/checkpoint/comem/rulin/cache/apptainer-sifs}"

# Embedded sandfleet controller: run the control plane on this job's head node.
# Standalone controllers only fit cpu_lowest (rulin's sole CPU QOS), where they were
# preempted 3x in 2h — twice tearing the durable state file — and each Slurm-on-k8s
# requeue changes the pod IP, orphaning the SANDFLEET_URL baked into this job's env.
# Co-location gives a preemption-free controller whose lifecycle and URL follow the
# trainer. Worker jobs still submit to cpu_lowest (preemption there is absorbed by
# the lease/heal design).
if [ "${SANDFLEET_EMBED_CONTROLLER:-0}" = "1" ]; then
    SF_EMBED_DIR="${SANDFLEET_EMBED_DIR:-/checkpoint/comem/rulin/sandfleet_rt/embed}"
    mkdir -p "${SF_EMBED_DIR}"
    SF_SRC="${SANDFLEET_SRC:-/checkpoint/comem/rulin/sandfleet-main-035559e}"
    # Overridable for cross-account hosts (Oscar): token dir, worker account/qos were
    # hardcoded to rulin/comem, which no other account can read or bill. Defaults
    # byte-identical for our own arms.
    SF_RT="${SANDFLEET_RT_DIR:-/checkpoint/comem/rulin/sandfleet_rt}"
    SF_WORKER_ACCOUNT="${SANDFLEET_WORKER_ACCOUNT:-comem}"
    SF_WORKER_QOS="${SANDFLEET_WORKER_QOS:-cpu_lowest}"
    SF_IP=$(hostname -i | awk '{print $1}')
    SF_PORT="${SANDFLEET_EMBED_PORT:-14411}"
    # The trainer env carries raw SANDFLEET_*_TOKEN vars for the backend; sandfleetd
    # refuses raw+_FILE together, so strip the raw forms from the daemon's env only.
    env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY \
        -u SANDFLEET_TOKEN -u SANDFLEET_CLIENT_TOKEN -u SANDFLEET_METRICS_TOKEN \
        SANDFLEET_TOKEN_FILE="$SF_RT/service.token" \
        SANDFLEET_CLIENT_TOKEN_FILE="$SF_RT/client.token" \
        SANDFLEET_METRICS_TOKEN_FILE="$SF_RT/metrics.token" \
        "$SF_SRC/.venv/bin/sandfleetd" \
        --host 0.0.0.0 --port "$SF_PORT" --advertise-host "$SF_IP" \
        --registry "${SF_EMBED_DIR}/controller-${SLURM_JOB_ID}.json" \
        --state-file "${SF_EMBED_DIR}/state-${SLURM_JOB_ID}.json" \
        --partition cpu --account "$SF_WORKER_ACCOUNT" --qos "$SF_WORKER_QOS" \
        --cpu-allocation-granularity 2 \
        --agent-advertise-mode route-ip \
        --agent-python "$SF_SRC/.venv/bin/python" \
        --worker-python "$SF_SRC/.venv/bin/python" \
        --source-root "$SF_SRC" \
        --log-dir "${SF_EMBED_DIR}" \
        --apptainer-binary "$(command -v apptainer)" \
        --step-start-rate 4 --step-start-burst 4 \
        --lease-ttl-seconds 1800 \
        --time-limit "${SANDFLEET_WORKER_TIME_LIMIT:-7-00:00:00}" \
        --enable-dynamic-resources \
        --dynamic-pool-slots-per-job 48 \
        --dynamic-worker-max-cpus 96 \
        --dynamic-worker-max-memory-mb 393216 \
        --dynamic-pool-max-capacity 640 \
        --dynamic-pool-idle-timeout-seconds 900 \
        --cleanup-on-exit \
        > "${SF_EMBED_DIR}/controller-${SLURM_JOB_ID}.out" 2>&1 &
    SANDFLEET_EMBED_PID=$!
    for _i in $(seq 1 60); do
        [ -s "${SF_EMBED_DIR}/controller-${SLURM_JOB_ID}.json" ] && break
        if ! kill -0 "${SANDFLEET_EMBED_PID}" 2>/dev/null; then
            echo "embedded sandfleetd died during boot; see ${SF_EMBED_DIR}/controller-${SLURM_JOB_ID}.out" >&2
            exit 1
        fi
        sleep 2
    done
    export SANDFLEET_URL="http://${SF_IP}:${SF_PORT}"
    echo "embedded sandfleet controller: ${SANDFLEET_URL} (pid ${SANDFLEET_EMBED_PID}, job ${SLURM_JOB_ID})"
fi
...
PY
        ...
        # (tmax-private#1): emitted keys are asserted against
        # dataclasses.fields(SWERLVanilluxSandboxEnvConfig); the dead vmvm backend
        # hard-fails here, the orphaned data knobs (SWERL_SKIP_DATA_BATCHES_ON_RESUME /
        # SWERL_EXCLUDE_*) fail fast at grpo_fast startup, and the watchdog runs this
        # same script as a preflight before any resubmit.
        TOOL_CONFIGS="$(
